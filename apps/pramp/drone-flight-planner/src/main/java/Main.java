...
class Main {
    ...
    /** Return the amount of energy required to make the trip without crashing. */
    static int calcDroneMinEnergy(int[][] route) {
        // your code goes here

        // Handle special cases by taking care of null and invalid input.
        if (route == null)
            return 0;
        for (int[] arr : route)
            if (arr.length != 3)
        ...
        // Handle normal cases, i.e. find the energy required to avoid a crash.
        int energy = 0;
        for (int i = 1; i < route.length; i++)
            if (route[i][2] > route[0][2] && (route[i][2] - route[0][2]) > energy)
                energy = route[i][2] - route[0][2];
        return energy;
    }
}
